...
class ParticleVizManager extends React.Component {
  ...
  updateSelectedModel(e) {
    ...
    for (let i = 0; i < models.length; i++) {
      if (
        models[i].name.toLowerCase().trim() ===
        e.target.text.toLowerCase().trim()
      ) {
  ...
  render() {
    if (isMobile || window.innerWidth < 992) {
      // if(true){
      return (
        <Container fluid style={{zIndex : 10,position:"relative"}}>
          ...
          <Collapse in={this.state.show_menu}>
            <Container fluid id={"col_content"} className={"mt-1"}>
              <Row className={`bg-light px-2 py-1`}>
                ...
                <Col xs={7}>
                  {" "}
                  <span className={"m-1"}>Background</span>{" "}
                </Col>
                <Col xs={{ span: 4, offset: 1 }}>
                  <BackgroundLayerManager
                    background_layer={this.props.background_layer}
                    map={this.props.map}
                    url={this.props.url}
                  />
                </Col>
              ...
              <Row className={`bg-light mb-1`}>
                <Col xs={12}>
                  ...
                  <ParticlesLayer
                    ...
                    _3D_render={this.state._3D_render}
                  />
                </Col>
              </Row>
              <Row className={`bg-light mb-1`}>
                ...
                this.setState({_3D_render : !this.state._3D_render})
              }}>
                {this.state._3D_render ? "3D" :"2D"}
              </button></Col>
              </Row>
            </Container>
          </Collapse>
          ...
          <nav className="navbar navbar-expand-md navbar-light bg-light justify-content-center">
            ...
            >
              <OverlayTrigger
                ...
                delay={{ show: 1, hide: 1 }}
                overlay={(props) => (
                  <Tooltip id="tooltip_home_icon" {...props}>
                    {" "}
                    Home
                  </Tooltip>
                )}
            ...
            {/* ---------- Particles menu ------------*/}
            {/* ---------- All options from particles ------------*/}
            <span className="navbar-brand align-middle" style={{ zIndex: 200 }}>
              {" "}
            ...
            <span className="navbar-brand align-middle">
              <OverlayTrigger
                ...
                delay={{ show: 1, hide: 1 }}
                overlay={(props) => (
                  <Tooltip id="tooltip_mod_sel" {...props}>
                    {" "}
                    Datasets
                  </Tooltip>
                )}
              >
                <Dropdown
                  ...
                  data-intro="Model Selection"
                  data-oz-position={chardin_offset + 10}
                >
                  <Dropdown.Toggle variant="info" size="sm">
                    {this.state.selected_model.name}
                  </Dropdown.Toggle>
